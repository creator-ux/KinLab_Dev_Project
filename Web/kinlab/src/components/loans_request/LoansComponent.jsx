...
import { HiOutlineArrowPath } from 'react-icons/hi2'; // Ícono para devolución
...
function LoansComponent() {
    ...
    // Inventario de componentes (id -> cantidad)
    ...
    useEffect(() => {
        const fetchSolicitudes = async () => {
            try {
                const data = await api.get('/api/loansComponents');
                setSolicitudes(data);
            } catch (err) {
    ...
    // Cargar inventario actual de componentes para mostrar cantidad total y su delta
    useEffect(() => {
        const fetchComponentes = async () => {
            try {
                ...
                const map = {};
                for (const c of data) {
                    map[c.id_componente] = c.cantidad;
                }
                setComponentesMap(map);
            } catch (err) {
                // No bloquear la vista de préstamos si falla el inventario
                console.error('Error al cargar inventario de componentes:', err);
            }
        };
        fetchComponentes();
    }, []);
    ...
    useEffect(() => {
        ...
        const refreshData = async () => {
            try {
                const [sols, comps] = await Promise.all([
                    api.get('/api/loansComponents'),
                    api.get('/api/component')
                ]);

                // Actualiza solicitudes
                setSolicitudes(Array.isArray(sols) ? sols : []);

                // Actualiza inventario de componentes (mapa id -> cantidad)
                const map = {};
        ...
        // Escucha actualizaciones globales disparadas desde otras vistas
        ...
        // Polling cada 5 segundos para sincronizar con la base de datos
    ...
    const handleUpdateRequest = async (id, nuevoEstado) => {
        try {
            await api.put(`/api/loansComponents/approve/${id}`, { estado_aprobacion: nuevoEstado });
            setSolicitudes(prev => prev.map(solicitud =>
                solicitud.id_prestamoC === id
    ...
    const handleReturnRequest = async (id) => {
        try {
            await api.put(`/api/loansComponents/return/${id}`);
            setSolicitudes(prev => prev.map(solicitud =>
                solicitud.id_prestamoC === id
        ...
        if (solicitud.estado_aprobacion === 0) {
            return (
                <div className="flex items-center justify-center space-x-3">
                    <button onClick={() => handleUpdateRequest(solicitud.id_prestamoC, 1)} title="Aprobar" className="p-2 rounded-full bg-green-100 text-green-700 hover:bg-green-600 hover:text-white transition-colors">
                        <FaCheck size={16} />
                    </button>
                    <button onClick={() => handleUpdateRequest(solicitud.id_prestamoC, 2)} title="Rechazar" className="p-2 rounded-full bg-red-100 text-red-700 hover:bg-red-600 hover:text-white transition-colors">
                        <FaTimes size={16} />
                    </button>
                </div>
            );
        ...
        if (solicitud.estado_aprobacion === 1 && solicitud.devuelto === 0) {
            return (
                <div className="flex justify-center">
                    <button onClick={() => handleReturnRequest(solicitud.id_prestamoC)} title="Confirmar Devolución" className="flex items-center justify-center px-3 py-1.5 rounded-full bg-blue-100 text-blue-700 hover:bg-blue-600 hover:text-white transition-colors text-sm font-semibold">
                        Devuelto
                    </button>
                </div>
            );
    ...
    const openIncidentModal = (solicitud) => {
        ...
        setIncidentForm(prev => ({
            ...prev,
            titulo: `Incidente - ${solicitud.nombre_componente}`,
            descripcion: '',
            id_laboratorio: null,
            accion_tomada: ''
        }));
    ...
    const submitIncident = async (e) => {
        ...
        try {
            ...
            await api.post('/api/incidents', {
                ...
                id_componente: selectedLoan.id_componente,
                id_equipo: null,
    ...
    const renderTableContent = () => {
        ...
        return currentItems.map((solicitud) => {
            ...
            const stockActual = componentesMap[solicitud.id_componente];
            ...
            return (
                <tr key={solicitud.id_prestamoC} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-5">{solicitud.nombre_componente}</td>
                        ...
                        {typeof stockActual !== 'number' ? (
                            ...
                            <div className="flex items-center justify-center gap-2">
                                ...
                                {isAceptado && solicitud.devuelto === 0 && (
                                    /* indica la cantidad que se desconto del inventario */
                                    <span className="text-xs px-2 py-1 bg-green-100 text-green-700 rounded"> {delta} en uso</span>
                                )}
                    ...
                    <table className="min-w-full w-full">
                        <thead>
                            <tr className="bg-gray-100 text-left text-gray-600 uppercase text-sm">
                                ...
                                <th className="py-3 px-5 font-semibold">Componente Solicitado</th>
